...
Status: **APPROVED FOR TEMPORARY PUBLIC SELF-SIGNED INTERNAL DEVELOPMENT PILOT**

Decision dates: 2026-08-23 (initial owner-only demo); 2026-08-24 (public internal-pilot exception)
...
The authorized owner expanded the development-device scope on 2026-08-23 to x64 Windows 10 version 1809 (build 17763) or later and Windows Server 2022. Microsoft documents Windows App SDK support back to Windows 10 version 1809 and support for Windows Server 2022, and documents .NET 10 x64 support for Windows Server 2022.

On 2026-08-24, the authorized owner approved temporary public distribution of the self-signed development companion through this public repository's GitHub Releases page for an internal development pilot. Each pilot user must independently decide to proceed, have local administrator rights, explicitly import only the public development certificate into `LocalMachine\TrustedPeople`, and explicitly install the MSIX through Windows App Installer. The web application and companion MUST NOT elevate, install a certificate, install or update the package, alter device policy, or claim that GitHub provenance provides Windows publisher trust.

This exception prohibits production or customer data, silent installation, automated certificate deployment, production use, and representation of the development publisher as a verified legal, Microsoft, or GitHub identity. Publicly trusted signing and managed deployment remain mandatory before broader rollout. Endpoint policy may block installation; users must not bypass that policy.
...
| Microsoft Edge Stable | `151.0.4129.101` at review | Installed Microsoft-signed executable | `CompanyName: Microsoft Corporation`; runtime version is compatibility-checked rather than bundled | Headed unauthenticated automation and ordinary isolated-profile handoff |
...
| Azure CLI Artifact Signing extension | `1.0.0` preview | Microsoft Corporation; official Azure CLI extension index; `https://azcliprod.blob.core.windows.net/cli-extensions/artifact_signing-1.0.0-py3-none-any.whl` | MIT; minimum Azure CLI `2.75.0`; SHA-256 `bbd04ad52426e69e9a24192f2fe0e2ed2db55ae61fb91b6b8b3a0303bdfd0c7f` | Previously installed for the superseded Public Trust design; not used to build, sign, or publish the self-signed development package |
| Azure Artifact Signing | Superseded; no certificate profile | Microsoft Azure service, stable ARM API `2025-10-13` | Microsoft managed signing service | Not used by the self-signed development package; resource cleanup remains a separate explicitly authorized Azure operation |
...
- Windows App SDK includes native/runtime packaging assets under custom Microsoft terms. The authorized Legal/OSS and Endpoint/Signing owner approved development packaging under those terms for the bounded internal pilot. Package identity, exact development publisher, non-exportable key, explicit local trust, signature verification, GitHub Release integrity, and rollback evidence remain required before an artifact is published.
...
- One-time certificate trust setup requires an independently chosen, explicitly elevated action to import the public `.cer` into `LocalMachine\TrustedPeople`. The package and companion request no elevation, service, scheduled task, local listener, broad filesystem root, browser extension, or normal-profile access. The release documentation must tell users to stop when they do not understand the development-certificate warning or when endpoint policy blocks installation.

## Rollback

The companion remains a separate repository project and does not enter the production container. Server and web integration default disabled. Rollback disables new ticket creation, marks the affected GitHub release unavailable, instructs each pilot administrator to remove the public certificate from `LocalMachine\TrustedPeople` and uninstall the package, deletes the owner's matching private key from `CurrentUser\My`, lets bounded tickets expire, and reverts focused application changes without touching saved projects, calculations, or unrelated Azure resources. The companion has no self-updater or downgrade bypass.

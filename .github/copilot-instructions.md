...
### Priority 0: Family Dinner Informational Coordination

- Read [FAMILY-DINNER.md](../FAMILY-DINNER.md) as the first repository operation for situational awareness and make a best-effort attempt to add one Flight Controller row and task entry. Family Dinner is informational only: a busy, missing, malformed, or stale board MUST NOT block isolated source work, validation, commits, authorized pushes, or automatic CI. Retry board updates later without waiting or starting a watcher solely for the board.
- Entries describe local worktrees or branches, expected paths, commands, processes, ports, cloud resources, and CI activity. They do not reserve source code, branches, refs, CI capacity, or permission; overlapping paths, target refs, CI runs, and advisory entries are not blockers. Use isolated worktrees and normal Git integration, and pause only the exact operation with a concrete process, port, generated-output, deployment/environment, cloud-resource, or unresolved Git conflict.
- Serialize edits to `FAMILY-DINNER.md` only with its **Board Write Mutex**: atomically create `.family-dinner.lock/`, immediately write the task ID to `.family-dinner.lock/owner`, re-read and narrowly update the board, validate, then delete only that owned lock and marker. If the lock exists, do not alter it; skip or defer the board update and continue all other work. Never hold or watch the mutex while doing task work, and never treat elapsed time alone as proof that another lock is stale.
- Keep the Flight Controller first with one row per reported active task. On each successful board write, refresh the snapshot, durations, CI ownership, and snapshot mutex ownership. Record only concrete blockers; the board, source overlap, a target ref, an integration queue, and another task's CI are never blockers. Show each task's reserved/active workflow-ref-short-SHA informationally, glow queued or in-progress runs as **✨ `workflow-running` ✨**, show `held at snapshot` for the mutex writer, and color priority as 🔴 P0, 🟠 P1, 🟡 P2, or 🟢 P3.
- Every agent normally edits only its own row and block. A protocol-maintenance task may repair the panel or remove board-created blockers while preserving factual metadata. Board availability and metadata freshness never determine whether implementation or delivery may continue.
- For GitHub Actions, reuse an observed run for the exact same workflow/ref/commit instead of manually duplicating it. Distinct commits and automatic CI runs may proceed independently. Record run intent, status, ID, and non-sensitive URL when practical, but never wait for another task's unrelated run merely because it appears on the board.
- Keep changes in focused, independently validated commits and push them as soon as they are ready and authorized. Multiple already-ready compatible commits may share one bounded fast-forward push while preserving commit boundaries and authorship, but agents MUST NOT wait solely to form a batch or for a board owner to clear. If a push loses a race, fetch, integrate normally, rerun affected checks, and retry.
- Coordinate only genuinely shared deployment, environment, process, port, generated-output, or cloud-resource mutations at that operational boundary. Family Dinner never authorizes development or production application deployment and never weakens exact-SHA, review, security, privacy, software-approval, rollback, or human-approval requirements.
- Before a final response, make one best-effort attempt to remove the completed task's row and block. If the mutex is busy, do not wait or withhold completed work; a later protocol-maintenance write may remove an entry only with explicit completion evidence. A failed board update does not block a handoff or final response.
...
### Priority 1: Development Application Deployment Authorization

- Never run, trigger, dispatch, retry, rerun, or otherwise initiate the development application deployment workflow unless the user explicitly requests that deployment in the current request. Do not infer deployment authorization from a request to implement, validate, test, push, investigate CI, prepare a deployment, or complete a feature.
- A development application deployment does not require a separate preview run. Each authorized run MUST target the current `main` commit, require successful CI for that exact commit, require the exact resource-group confirmation, build or reuse locked dependency and application images for that commit, and complete a deletion-free Bicep what-if with no foundation changes before it mutates Azure.
- Authorization applies only to the specifically requested run and does not authorize retries or later runs. Without explicit authorization, use local checks and non-deploying CI validation and stop before dispatching the development application workflow.
...
- The development application deployment workflow owns dependency-image creation or verified reuse, exact-commit application-image creation or verified reuse, and digest resolution. In the same authorized run, it MUST verify successful exact-commit CI, lock and verify both images, complete a deletion-free Bicep what-if with no foundation changes before mutation, deploy the resolved application digest, and complete the existing identity, authorization, health, readiness, persistence, and version checks.

...
- Keep the Flight Controller as the first operational section with exactly one row per active task. On every board write, refresh its snapshot and all displayed durations; change a row's status-change time only when its status or reason changes; and identify blockers by exact task ID, `external: <reason>`, or `none`. Render queued or in-progress CI/CD ownership as **✨ `workflow-running` ✨** until the run becomes terminal or is handed off, and color-code priority as 🔴 P0 urgent blocker, 🟠 P1 high, 🟡 P2 normal, or 🟢 P3 opportunistic. Operational values belong to the task owner except for protocol bootstrap or repair from existing metadata.

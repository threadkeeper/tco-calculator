      ...
      {#each projects as project (readString(project, 'id'))}
        ...
        <div class="table-row">
          ...
          <span class="row-actions">
            <button type="button" onclick={() => onopen(id)} aria-label={`Open ${name}`} title="Open project"><ArrowRight size={17} /></button>
            <button class="delete" type="button" onclick={() => ondelete(id, name)} aria-label={`Delete ${name}`} title="Delete project"><Trash2 size={16} /></button>
          </span>
        </div>
      {/each}
    </div>
...
<style>
  .table-body { min-width: 1160px; }
  .table-row { display: grid; grid-template-columns: 1.8fr 0.75fr 1fr 1fr 1fr 0.65fr 1fr 1fr 0.7fr; align-items: center; gap: 14px; min-height: 58px; padding: 8px 14px; color: #4b5f63; border-bottom: 1px solid #e0e6e7; font-size: 0.8rem; }
  .table-row:last-child { border-bottom: 0; }
  .table-row:hover { background: #f6f9f8; }
  .project-name { overflow: hidden; padding: 4px 0; color: #075e71; background: transparent; border: 0; font: 700 0.88rem/1.3 Bahnschrift, sans-serif; text-align: left; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
  .table-row strong { overflow-wrap: anywhere; color: #263d42; font-weight: 650; }
  .row-actions { display: flex; gap: 4px; }
  .row-actions button { display: grid; width: 31px; height: 31px; place-items: center; padding: 0; color: #31535a; background: #fff; border: 1px solid #b9c7c9; border-radius: 4px; cursor: pointer; }
  .row-actions .delete { color: #a62a20; }
</style>
...
- The preceding browser rule has one approved exception for the separately installed Azure Calculator companion defined in section 5.4. The companion is a public native client, uses MSAL.NET with Windows Web Account Manager, requests only the approved delegated Calculator launch scope for this API, and sends its access token only to the exact allowlisted TCO API origin. The web frontend still MUST NOT add MSAL or receive, store, refresh, or forward that token. Container Apps authentication remains the token-validation boundary; Rust additionally requires the delegated scope, approved `azp`/`appid`, delegated-user identity, and the same `tid` plus `oid` owner key on companion routes.
...
### 5.4 Attended Azure Pricing Calculator Companion

The approved temporary internal-development-pilot workflow supports x64 Windows 10 version 1809 (build 17763) or later and Windows Server 2022. It MAY transfer a saved, clean, server-authoritative EC2, RDS, or on-premises calculation to the public Azure Pricing Calculator through a separately installed and signed WPF application targeting pinned .NET 10. SQL Pay As You Go projects, guest drafts, dirty or unsaved projects, stale/unavailable pricing, partially mapped projects, non-USD projects, and projects over the approved 25-item release cap MUST be rejected. The self-signed development package MAY be distributed publicly through this repository's GitHub Releases page for use by internal pilot users who independently choose to trust and install it with local administrator rights. This approval does not cover production, customer data, silent installation, automated certificate deployment, or broader rollout.

The hosted application MUST preserve its single Rust/Svelte image and one-Container-App topology. The desktop companion, .NET runtime, Edge, Playwright, profiles, and packaging tools MUST NOT enter the production image or run as another Azure service, worker, job, sidecar, function, or server-side browser.

The browser creates a cryptographically random UUID and sends only that UUID plus protocol version to an owner-scoped create endpoint with the current project ETag. A custom URI contains only the canonical UUID and protocol version. The server reloads the persisted project using the platform-derived `tid` plus `oid`, validates eligibility and the persisted calculation revision, and stores a bounded immutable manifest in the existing owner-partitioned Cosmos container. The ready/claim window is ten minutes.

The companion claims the manifest using the approved multitenant workforce public native-client registration, WAM, and one delegated API scope. The API MUST validate the platform-authenticated audience and user plus route-level `scp`, approved `azp`/`appid`, delegated-user identity, `tid`, and `oid`. Custom-URI possession is never authorization. The ready-to-claimed transition MUST use an atomic Cosmos ETag condition. After strict manifest acceptance, the companion acknowledges receipt; the server immediately purges the manifest and source binding and MAY retain only a minimal consumed idempotency tombstone for 24 hours. There is no server browser worker, queue, scheduler, progress state, browser-facing status endpoint, web polling, or web-driven cancellation after activation.

The companion MUST use a new app-owned ephemeral profile and installed Microsoft Edge Stable. It automates only visible Calculator controls while unauthenticated, reads every configured value back, reloads once, and verifies again. It MUST NOT use the normal Edge profile, private Calculator APIs, direct IndexedDB writes, injected page scripts, copied storage state, remote-debugging TCP, screenshots, traces, videos, request/response capture, credential handling, CAPTCHA bypass, or anti-automation evasion in release operation.

Before the user is invited to authenticate, the companion MUST close and dispose Playwright and its controlled Edge process. It then starts ordinary Edge with the same isolated profile and no automation/debugging connection. After this handoff, the companion may retain only the process handle and owned profile path for cleanup; it MUST NOT inspect page content, URLs, DOM, network, storage, downloads, credentials, cookies, tokens, or user input. Microsoft sign-in, agreement selection, Save, Share, and Export remain direct user actions on Microsoft's origin. If the unauthenticated estimate state does not survive this handoff and interactive sign-in, the feature stops; authenticated-profile automation is not approved.

The companion window owns authentication, automation, verification, handoff, cancellation, and cleanup progress. The web page provides only immediate open/install guidance and MUST NOT infer local progress. Normal Edge exit triggers bounded deletion of the isolated profile; locked or crash-abandoned profiles are retried only within the app-owned root at next companion start. Cleanup MUST reject reparse points and any path not created and marked by the companion. The accepted residual risk is that logical deletion cannot have a guaranteed wall-clock deadline after a companion or workstation crash without a separately approved background mechanism.

The manifest uses the persisted saved project name as the Calculator estimate name and each persisted workload name as its Calculator line label, plus only the target fields listed in `THIRD-PARTY-DATA-EGRESS.md`. The launch request MUST NOT supply or override those names. The manifest MUST exclude project descriptions, server names, source SKUs/capacity/costs and infrastructure identifiers, owner/tenant identifiers, application discounts, parity adjustments, expected totals, commercial agreements, and credentials. Calculator output is an estimate, not a quote, entitlement, discount promise, or licensing determination.

The server feature flag MUST default off. Exact manifest/protocol/Calculator contract versions, minimum companion version, per-owner rate limits, drift circuit breaker, self-signed pilot MSIX validation, versioned GitHub Release distribution, package provenance, dependency/license/vulnerability review, and synthetic/security/accessibility tests are development-pilot requirements. The non-exportable private key MUST remain in the owner's `CurrentUser\My` store and MUST NOT enter a PFX, repository, GitHub secret, or release asset. Only the public certificate may be distributed. Each pilot user must independently choose to import it, through an explicit elevated setup step, into `LocalMachine\TrustedPeople`; it MUST NOT be installed as a root CA. GitHub identity and artifact attestations establish source provenance only and MUST NOT be represented as Windows publisher trust. The application runs `asInvoker` and MUST NOT elevate, install certificates or packages, alter device policy, invoke `ms-appinstaller:`, or bypass endpoint controls. Unsupported or policy-blocked devices stop with install guidance. The authorized owner approved the initial owner-device exception on 2026-08-23 and temporary public self-signed distribution for the internal development pilot on 2026-08-24. Publicly trusted signing and managed deployment are mandatory before broader rollout. Development deployment remains subject to exact-commit CI and repository workflow gates.
...
### 7.6 Excel Workbook Result Export

After a successful calculation, the user MAY download a native `.xlsx` workbook. The `TCO Results` worksheet MUST contain the current project context, derived MI target, exact server-returned component costs, savings, parity values, formula version, and pricing snapshot IDs with one logical row per resource. For every resource, it MUST include the server-returned SQL data, persistent EBS, and derived Azure storage quantities used by the calculation.

The `Inputs` worksheet MUST contain the normalized project request used for target mapping and calculation: project name and description, selected price snapshot IDs, every project setting including nested SQL PAYG settings, every source-resource input, and every nested EBS volume input. It MUST expose each leaf value in a filterable row with its section, workload, stable input path, and exact value. Null inputs MUST remain represented as blank values, and empty collections MUST remain explicit. Both worksheets MUST preserve exact decimal text and MUST NOT recalculate financial values in the browser. The `TCO Results` grid MUST use native worksheet formatting to mirror the application's dark-mode calculation detail grid: Aptos body text, Bahnschrift headers, matching semantic fills and text accents, dark cell borders with default gridlines hidden, two header rows with merged group headers, and equivalent column widths and row heights. Neither worksheet may freeze panes. Preserve filters and print layout.

The browser MUST create the workbook locally from the project already visible to the current user and the latest calculation response. Do not add an export API, server-side export storage, upload, analytics event, or third-party egress. The export MUST exclude owner identifiers, identity claims, display names, email/contact consent, ETags, capability secrets, and other authorization metadata. Store formula-like values as explicit text cells, use a sanitized `.xlsx` filename, and make clear that the downloaded file contains confidential business data governed by the user's managed-device and storage controls.
...
The explicit `sql_data_gb_per_instance` value and each EBS volume's provisioned capacity are independent inputs and MUST remain independently editable. Adding, removing, or editing an EC2 volume MUST NOT overwrite the SQL data value. For EC2, the server derives `persistent_ebs_gb_per_instance` as the exact sum of all non-ephemeral volume capacities and derives `azure_storage_gb_per_instance = sql_data_gb_per_instance + persistent_ebs_gb_per_instance`. The combined Azure quantity is authoritative for SQL MI storage-capacity selection and Azure storage pricing. Calculation results, the detail grid, explanations, and Excel workbook export MUST expose all three quantities.
...
- Client-side Excel workbook result export is an explicit user-initiated disclosure to the user's device. Generate it only from the authorized project already loaded in that browser, exclude personal and authorization metadata, store formula-like values as text cells, and do not send or retain the file server-side.

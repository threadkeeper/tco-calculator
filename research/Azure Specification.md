...
The manifest uses anonymous ordinal labels and only the target fields listed in `THIRD-PARTY-DATA-EGRESS.md`. It MUST exclude project/workload names, source SKUs/capacity/costs, owner/tenant identifiers, application discounts, parity adjustments, expected totals, commercial agreements, and credentials. Calculator output is an estimate, not a quote, entitlement, discount promise, or licensing determination.

The server feature flag MUST default off. Exact manifest/protocol/Calculator contract versions, minimum companion version, per-owner rate limits, drift circuit breaker, owner-only self-signed MSIX validation, versioned GitHub Release distribution, package provenance, dependency/license/vulnerability review, and synthetic/security/accessibility tests are development-demo requirements. The non-exportable private key MUST remain in the owner's `CurrentUser\My` store and MUST NOT enter a PFX, repository, GitHub secret, or release asset. Only the public certificate may be imported, through an explicit elevated setup step, into `LocalMachine\TrustedPeople`; it MUST NOT be installed as a root CA. GitHub identity and artifact attestations establish source provenance only and MUST NOT be represented as Windows publisher trust. The application runs `asInvoker` and MUST NOT alter device policy, invoke `ms-appinstaller:`, or bypass endpoint controls. Unsupported or policy-blocked devices stop with install guidance. The authorized owner approved this narrow development exception and its documented residual risk on 2026-08-23. Production or third-person distribution requires a new signing approval. Development deployment remains subject to exact-commit CI and repository workflow gates.

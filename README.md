...
The development application uses the manual **Deploy Azure application** workflow. Any agent may dispatch or retry it for the current `main` commit after that exact commit's CI succeeds. One serialized run builds or reuses locked dependency and application images, validates an application-only deletion-free Bicep `what-if`, rejects a commit superseded before mutation, deploys the image digest, and verifies identity, health, readiness, persistence, and version. It has no separate preview operation or preview run ID. Foundation and any future production deployment remain separately authorized operations.

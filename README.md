...
The development application uses the manual **Deploy Azure application** workflow in two separately authorized phases. `build-preview` requires successful CI for the exact current `main` commit, builds or reuses locked dependency and application images, and validates an application-only deletion-free Bicep `what-if`. After that run is reviewed, `deploy` requires its exact run ID and the resource-group confirmation, rejects a superseded commit, reuses the locked application digest without rebuilding, and verifies identity, health, readiness, persistence, and version. Preview, deployment, retries, foundation changes, and any future production deployment remain explicitly authorized operations.
